...
class Vehicle(models.Model):
    ...
    next_maintenance = fields.Date(string="Next maintenance", compute="_compute_next_maintenance", store=True)
    vehicle_category = fields.Selection(
        [("car", "Car"), ("motorcycle", "Motorcycle"), ("truck", "Truck"), ("other", "Other")],
        default="car",
        string="Vehicle type",
        required=True
    )
    ...
    @api.depends("maintenance_ids.departure_date")
    ...
    def _compute_next_maintenance(self):
        for record in self:
            if record.last_maintenance:
                record.next_maintenance = record.last_maintenance + relativedelta(years=1)
